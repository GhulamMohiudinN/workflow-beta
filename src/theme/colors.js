export const colors = {
  primary: {
    50: "#eff6ff",
    100: "#dbeafe",
    200: "#bfdbfe",
    300: "#93c5fd",
    400: "#60a5fa",
    500: "#3b82f6",
    600: "#2563eb",
    700: "#1d4ed8",
    800: "#1e40af",
    900: "#172554",
  },
  secondary: {
    50: "#f0fdfa",
    100: "#ccfbf1",
    200: "#99f6e4",
    300: "#5eead4",
    400: "#2dd4bf",
    500: "#14b8a6",
    600: "#0d9488",
    700: "#0f766e",
    800: "#115e59",
    900: "#134e4a",
  },
  success: {
    50: "#f0fdf4",
    100: "#dcfce7",
    200: "#bbf7d0",
    300: "#86efac",
    400: "#4ade80",
    500: "#22c55e",
    600: "#16a34a",
    700: "#15803d",
    800: "#166534",
    900: "#145231",
  },

  warning: {
    50: "#fffbeb",
    100: "#fef3c7",
    200: "#fde68a",
    300: "#fcd34d",
    400: "#fbbf24",
    500: "#f59e0b",
    600: "#d97706",
    700: "#b45309",
    800: "#92400e",
    900: "#78350f",
  },

  danger: {
    50: "#fef2f2",
    100: "#fee2e2",
    200: "#fecaca",
    300: "#fca5a5",
    400: "#f87171",
    500: "#ef4444",
    600: "#dc2626",
    700: "#b91c1c",
    800: "#991b1b",
    900: "#7f1d1d",
  },

  info: {
    50: "#eff6ff",
    100: "#dbeafe",
    200: "#bfdbfe",
    300: "#93c5fd",
    400: "#60a5fa",
    500: "#3b82f6",
    600: "#2563eb",
    700: "#1d4ed8",
    800: "#1e40af",
    900: "#1e3a8a",
  },

  neutral: {
    0: "#ffffff",
    50: "#f8fafc",
    100: "#f1f5f9",
    200: "#e2e8f0",
    300: "#cbd5e1",
    400: "#94a3b8",
    500: "#64748b",
    600: "#475569",
    700: "#334155",
    800: "#1e293b",
    900: "#0f172a",
  },
  background: "#f6f8fb",
  backgroundSecondary: "#eef3f8",
  surface: "#ffffff",
  surfaceHover: "#f8fafc",
  border: "#e2e8f0",
  borderDark: "#cbd5e1",
  text: {
    primary: "#111827",
    secondary: "#64748b",
    tertiary: "#94a3b8",
    inverse: "#ffffff",
  },
  shadow: "0 10px 30px rgba(15, 23, 42, 0.06)",
  shadowLg: "0 18px 45px rgba(15, 23, 42, 0.1)",
};

export const cssVariables = {
  "--color-primary": colors.primary[600],
  "--color-primary-hover": colors.primary[700],
  "--color-secondary": colors.secondary[600],
  "--color-success": colors.success[500],
  "--color-warning": colors.warning[500],
  "--color-danger": colors.danger[500],
  "--color-info": colors.info[600],
  "--color-bg": colors.background,
  "--color-bg-soft": colors.backgroundSecondary,
  "--color-surface": colors.surface,
  "--color-surface-hover": colors.surfaceHover,
  "--color-border": colors.border,
  "--color-border-strong": colors.borderDark,
  "--color-text": colors.text.primary,
  "--color-muted": colors.text.secondary,
  "--color-faint": colors.text.tertiary,
  "--shadow-card": colors.shadow,
  "--shadow-popover": colors.shadowLg,
};

export default colors;
