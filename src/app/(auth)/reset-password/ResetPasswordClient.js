...
export default function ResetPasswordClient() {
  // ── Read token directly from the URL on the client ──────────────────────
  // This is the reliable approach in Next.js 15 where searchParams is async
  // on the server. useSearchParams() is always synchronous on the client.
  ...
  const fieldCls = (hasError) =>
    `mt-1 block w-full rounded-lg border px-3 py-3 text-sm text-[var(--color-text)] bg-white focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] transition-all ${
        ...
        <div className="bg-white rounded-xl shadow-[var(--shadow-popover)] border border-[var(--color-border)] p-8">
          ...
          {success ? (
            <div className="flex flex-col items-center text-center py-4">
              ...
              <p className="text-sm font-semibold text-[var(--color-success)]">{success}</p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-3 top-10 text-[var(--color-faint)] hover:text-[var(--color-muted)]"
